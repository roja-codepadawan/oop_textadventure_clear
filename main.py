...
def dungeon():
  global x
  if x == 0:
    print(f'Welcome to DUNGEON!')
    x += 1
  time.sleep(2)
  print(f'\nYou are at DUNGEON level '+ str(lv) + f'\nFinish the quiz to go to the next level') #!!!
  time.sleep(4)
  ...
  def monster(n):
    ...
    for num1 in range(n):
      ...
      if score == lv:
        print(f'nYou finished level ' + str(lv))
        time.sleep(2)
        lv = lv + 1
        if lv == 11:
          game_over()
          print(f'\nYour current level is ' + str(lv))
          time.sleep(2)
          print(f'\nHP +10, MP +10, ATK +10')
          time.sleep(2)
          HP += 10
...
def game_over():
    ...
    if lv <= 0:
        ...
        print("Good job! You finished the game! You are very patient!")
        exit()
    else:
        print("\n Your HP = 0")
        print("\nGame Over!"
